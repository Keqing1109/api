...
def is_super_buyer(bvol, avol):
    return bvol / avol <= 0.7

def is_super_seller(bvol, avol):
    return avol / bvol <= 0.7
    ...
    while status == 'ACTIVE':
        for i in range(3):
            ...
            if is_super_buyer(bid_vol, ask_vol):
                resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 1.5, 'price': best_bid_price - 0.6, 'action': 'BUY'})
                print('A SUPER BUYERS MARKET FOR ' + str (ticker_symbol) + '! Longing ' + str(ORDER_LIMIT * 1.5) + ' at ' + str(best_bid_price - 0.5))
            elif is_super_seller(bid_vol, ask_vol):
                resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 1.5, 'price': best_ask_price + 0.6, 'action': 'SELL'})
                print('A SUPER BUYERS MARKET FOR ' + str (ticker_symbol) + '! Shorting ' + str(ORDER_LIMIT * 1.5) + ' at ' + str(best_ask_price - 0.5))
            else:
                ...
                resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT, 'price': best_ask_price + 0.5, 'action': 'SELL'})
