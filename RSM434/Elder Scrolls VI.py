...
def over_protection(limit):
    ...
    if threshold > 0.85: #If the total position reaches 95% of the limit
...
def dump_trump():
    ...
    for item in securities:
        position = item['position']
        if position > 0.0: #Dump long position if the underlaying ticker has positive position
            resp = s.post(u_orders, params = {'ticker': item['ticker'], 'type': 'MARKET', 'quantity': abs(position) * 0.6, 'action': 'SELL'})
            print(str(item['ticker']) + ' getting regularly short dumped')
        elif position < 0.0: #Dump short position if the underlaying ticker has negative position
            resp = s.post(u_orders, params = {'ticker': item['ticker'], 'type': 'MARKET', 'quantity': abs(position) * 0.6, 'action': 'BUY'})
            print(str(item['ticker']) + ' getting regularly long dumped')
    ...
    while status == 'ACTIVE':
        for i in range(3):
            ...
            if is_super_buyer(bid_vol, ask_vol):
                ...
                print('A SUPER BUYERS MARKET FOR ' + str (ticker_symbol) + '! Longing ' + str(ORDER_LIMIT * 1.3) + ' at ' + str(best_bid_price - 0.03))
                ...
                print('A SUPER BUYERS MARKET FOR ' + str (ticker_symbol) + '! Shorting ' + str(ORDER_LIMIT * 1.3) + ' at ' + str(best_ask_price + 0.03))
            elif is_competitive(best_bid_price, best_ask_price):
                #To fix the problem of 0 trade in RY when spread is extremely narrow
                resp = s.post(u_orders, params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 0.9, 'price': best_bid_price - 0.01, 'action': 'BUY'})
                resp = s.post(u_orders, params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 0.9, 'price': best_ask_price + 0.01, 'action': 'SELL'})
                print('Low spread for ' + str(ticker_symbol) + ' long & shorting at a reduced price')
            else:
                resp = s.post(u_orders, params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT, 'price': best_bid_price - 0.05, 'action': 'BUY'})
                resp = s.post(u_orders, params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT, 'price': best_ask_price + 0.05, 'action': 'SELL'})
