...
def main():
    ...
    while status == "ACTIVE":
        ...
        # ADD trading logic - compared valuation to market prices; buy if valuation > market price; sell if valuation < market price
        # modify the valuations! the sample is the "midpoint" estimate, assumes that the estiamtes are the middle of potential EPS
        # the midpoint assumption is especially problematic for institutional ownerhip
        # 1) add an error term around the midpoint valuation; e.g. valuatiuon is $35, you add 20%, or $5, or something; margin of error shrink over the case
        # 2) calculate a min and max, be aware of the weighting scheme - if the DDM < PE model valuation, properly add to min or the max
        # if the DDM < PE valuation, the  max institutional owenship would go with the min estimate; if DDM > PE, max institutional ownership woudl go with max estimate
